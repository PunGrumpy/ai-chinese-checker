...
class Game:
    ...
    def getValidMoves(self, startPos: tuple, playerNum: int, playerCount: int):
        ...
        if playerCount == 2:  
            ...
            start_zone = 2 if playerNum == 1 else 3 if playerNum == 2 else 5 if playerNum == 3 else 6
            end_zone = 4 if playerNum == 1 else 5 if playerNum == 2 else 2 if playerNum == 3 else 3
        elif playerCount == 6:
        ...
        for i in copy.deepcopy(moves):
            ...
            if (i not in ZONE_COOR[1]) and (i not in ZONE_COOR[2]) and (i not in ZONE_COOR[3]) and (i not in ZONE_COOR[4]) and (i not in ZONE_COOR[5]) and (i not in ZONE_COOR[6]) and (i not in NEUTRAL_COOR):
                while i in moves:
                    moves.remove(i)
    ...
    def checkWin(self, playerNum: int, playerCount: int):
        ...
        for i in ZONE_COOR[end_zone]:
            ...
            if isinstance(self.board[i], Piece) and self.board[i].getPlayerNum() == playerNum: check = True
        return check
    ...
    def getBoardState(self, playerNum: int):
        ...
        state = dict()
        for i in self.board:
            state[obj_to_subj_coor(i, playerNum,self.playerCount)] = (0 if self.board[i] == None else int(self.board[i].getPlayerNum()))
        return state
    ...
    def getBoolBoardState(self, playerNum: int):
        ...
        state = dict()
        for i in self.board:
            state[obj_to_subj_coor(i, playerNum,self.playerCount)] = (self.board[i] != None)
        return state
    ...
    def allMovesDict(self, playerNum: int):
        ...
        for p in self.pieces[playerNum]:
            ...
            moves[p_subj_coor] = [obj_to_subj_coor(i, playerNum,self.playerCount) for i in p_moves_list]
    ...
    def drawCircles(self, window:pygame.Surface, playerNum: int, playerCount: int):
        for obj_coor in self.board:
            ...
            if isinstance(self.board[obj_coor], Piece):
                ...
                pygame.draw.circle(window, PLAYER_COLORS[get_player_zone(self.board[obj_coor].getPlayerNum(),playerCount)], c, self.circleRadius-2)
    ...
    def drawPolygons(self, window: pygame.Surface, playerTurn: int, playerCount: int, playerNum: int=1):
        #center hexagon
        ...
        if playerZone == 0:
            pygame.draw.polygon(window, YELLOW, (add(self.centerCoor,mult(h2c((0,-4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((4,-4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((4,-8)), self.unitLength+highlight_border))))
        elif playerZone == 1:
            pygame.draw.polygon(window, YELLOW, (add(self.centerCoor,mult(h2c((-4,0)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((-4,-4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((0,-4)), self.unitLength+highlight_border))))

        elif playerZone == 2:
            pygame.draw.polygon(window, YELLOW, (add(self.centerCoor,mult(h2c((-8,4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((-4,4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((-4,0)), self.unitLength+highlight_border))))

        elif playerZone == 3:
            pygame.draw.polygon(window, YELLOW, (add(self.centerCoor,mult(h2c((-4,8)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((-4,4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((0,4)), self.unitLength+highlight_border))))

        elif playerZone == 4:
            pygame.draw.polygon(window, YELLOW, (add(self.centerCoor,mult(h2c((0,4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((4,4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((4,0)), self.unitLength+highlight_border))))

        else:
            pygame.draw.polygon(window, YELLOW, (add(self.centerCoor,mult(h2c((4,0)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((8,-4)), self.unitLength+highlight_border)), add(self.centerCoor,mult(h2c((4,-4)), self.unitLength+highlight_border))))


        pygame.draw.polygon(window, BLACK, (add(self.centerCoor,mult(h2c((-4, 8)), self.unitLength+border)), add(self.centerCoor,mult(h2c((0, 4)), self.unitLength+border)), add(self.centerCoor,mult(h2c((4, 4)), self.unitLength+border)), 
                                                add(self.centerCoor,mult(h2c((4, 0)), self.unitLength+border)), add(self.centerCoor,mult(h2c((8, -4)), self.unitLength+border)), add(self.centerCoor,mult(h2c((4, -4)), self.unitLength+border)), 
                                                add(self.centerCoor,mult(h2c((4, -8)), self.unitLength+border)), add(self.centerCoor,mult(h2c((0, -4)), self.unitLength+border)), add(self.centerCoor,mult(h2c((-4, -4)), self.unitLength+border)), 
                                                add(self.centerCoor,mult(h2c((-4, 0)), self.unitLength+border)), add(self.centerCoor,mult(h2c((-8, 4)), self.unitLength+border)), add(self.centerCoor,mult(h2c((-4, 4)), self.unitLength+border)), ))
        ...
        pygame.draw.polygon(window, colors[1], (add(self.centerCoor,mult(h2c((-4,0)), self.unitLength)), add(self.centerCoor,mult(h2c((-4,-4)), self.unitLength)), add(self.centerCoor,mult(h2c((0,-4)), self.unitLength))))
        pygame.draw.polygon(window, colors[2], (add(self.centerCoor,mult(h2c((-8,4)), self.unitLength)), add(self.centerCoor,mult(h2c((-4,4)), self.unitLength)), add(self.centerCoor,mult(h2c((-4,0)), self.unitLength))))
        pygame.draw.polygon(window, colors[3], (add(self.centerCoor,mult(h2c((-4,8)), self.unitLength)), add(self.centerCoor,mult(h2c((-4,4)), self.unitLength)), add(self.centerCoor,mult(h2c((0,4)), self.unitLength))))
        pygame.draw.polygon(window, colors[4], (add(self.centerCoor,mult(h2c((0,4)), self.unitLength)), add(self.centerCoor,mult(h2c((4,4)), self.unitLength)), add(self.centerCoor,mult(h2c((4,0)), self.unitLength))))
        pygame.draw.polygon(window, colors[5], (add(self.centerCoor,mult(h2c((4,0)), self.unitLength)), add(self.centerCoor,mult(h2c((8,-4)), self.unitLength)), add(self.centerCoor,mult(h2c((4,-4)), self.unitLength))))
        
        pygame.draw.polygon(window, WHITE, (add(self.centerCoor,mult(h2c((-4, 4)), self.unitLength)), add(self.centerCoor,mult(h2c((0, 4)), self.unitLength)), add(self.centerCoor,mult(h2c((4, 0)), self.unitLength)), 
                                                add(self.centerCoor,mult(h2c((4, -4)), self.unitLength)), add(self.centerCoor,mult(h2c((0, -4)), self.unitLength)), add(self.centerCoor,mult(h2c((-4, 0)), self.unitLength)), ))
